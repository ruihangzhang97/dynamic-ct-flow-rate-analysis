...
def mini_map(linepts, pixel_grid):
    ...
    sag1 = [-9.19, 16.66]
    sag2 = [0, len(pixel_grid[0])]
    cor1 = [-10.76, 15.10]
    cor2 = [0, len(pixel_grid[0][0])]
    ax1 = [210.05, 234.05]
    ax2 = [0, len(pixel_grid)]
...
def a_plane_over_time(pixelgrid4D, TEST, choose, size):
...
def a_plane_over_time_LOTS(folder, TEST, choices, all_time, how_many, size, here, speed):
    ...
    for i in choices:
        ...
        gif, saveable = a_plane_over_time(pixelgrid4D, TEST, choose, size)
...
def remove_bad_gifs(typee, removers, here):
    here = "/Users/felicialiu/Desktop/summer2022/coding/runthrough2/gif/"
...
def this_slice_average_rad(cont, tot_centres):
...
def the_whole_polarthing(gif_path, parent_dir, speed):
    ...
    # here = "/Users/felicialiu/Desktop/summer2022/coding/polarfiller/AORTAslice90frames2/"
    ...
    # centre = [100, 130] # THIS IS USER INPUT
...
def run_tics_for_all_gifs(parent_dir, speed):
    ...
    for name in names:
        ...
        print("*******\nWORKING ON", name)
        rad = the_whole_polarthing(gif_path, parent_dir, speed)
        rads.append(rad)
...
def plot_ALL_tics(colours, typee, ALL_vecs, names, here, use="", save=False, showw=True):
...
def vecs_to_times_converter_neighbour(ALL_vecs):
    times = [0.0]
    for i in range(0, len(ALL_vecs)-1):
        times.append(stalker(ALL_vecs[i], ALL_vecs[i+1]))
    
    # THEN EACH ELEMENT SHOULD BE THE SUM OF THE TIMES BEFORE IT
    times = np.array(times)
    new_times = []
    for i in range(1, len(times)+1):
        new_times.append(float("%.1f" % np.ndarray.sum(times[:i])))

    return new_times

# just to make sure it actually looks better?
# correct all the ALL_vecs
def plot_sanity_check_on_times(times, ALL_vecs):
    new = []
...
def plot_lengths_times_velocities(namee, lengths, times, here, save=False, showw=True):
    ...
    print(namee +"average good velocities: " + str(avgvel) + " [mm/s]")
    print(namee +"regression velocity: " + str(round(b[1])) + " [mm/s]")

    return avgvel, round(b[1])

# full velocity calculations
def running_velocity_extraction(TEST, parent_dir, typee, save=False, showw=True):
    ...
    plot_ALL_tics(colours, typee, ALL_vecs, names, here, "ALL_TICS", save, showw)
    ...
    plot_ALL_tics(colours, typee, ALL_vecs, names, here, "SMOOTH_TICS", save, showw)
    ...
    new = plot_sanity_check_on_times(times, ALL_vecs)
    plot_ALL_tics(colours, typee, new, names, here, "ffSHIFTED_TICS", save, showw)
    ...
    times = vecs_to_times_converter_neighbour(ALL_vecs)
    new = plot_sanity_check_on_times(times, ALL_vecs)
    plot_ALL_tics(colours, typee, new, names, here, "nfSHIFTED_TICS", save, showw)
...
how_many = 100 # capped at 151 if all_time == False
size = 200 # aorta will use bigger 
choices = choosing(2, 120, 8)
a_plane_over_time_LOTS(timebody_folder, TEST, choices, all_time, how_many, size, here, speed)
...
# avgrad = run_tics_for_all_gifs(parent_dir, speed)
# print("you need 'average vessel radius [pixels]' for next step:", avgrad)
...
# v = running_velocity_extraction(TEST, parent_dir, typee, True, False)
...
# avg_vel = 122 # mm/s
# radpix = 29 # pixels
# flow1, flow2 = flowrate(avg_vel, radpix)
# print("flowrate for", typee, "is:", flow1, "[L/min] OR", flow2, "[ml/s]")
