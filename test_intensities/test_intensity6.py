...
def a_plane_over_time_LOTS(parent_dir, folder, TEST, spacing, all_time, size, here, speed, typee):
...
def plot_two_cut(time, vec, rad, size):
...
def plot_two_no_cut(time, vec, rad, size):
    fig, ax1 = plt.subplots()
    title = "Cropping Intensity Time Curves (TICs) - Case 1 CABG Project"
    ax1.set_title(title)
    ax1.set_xlabel('time of perfusion acquisition [s]')
...
def value_cropper(time, vec, rad, size):

    crop = plot_two_cut(time, vec, rad, size)
    print(crop)
    newtime = time[crop[0]+1 : crop[1]]
    newvec = vec[crop[0]+1 : crop[1]]
    newrad = rad[crop[0]+1 : crop[1]]
    plot_two_no_cut(newtime, newvec, newrad, size)

    return newtime, newvec, newrad, crop

# let's user define bounds to crop all images
def timecrop_bad(ALL_times, ALL_vecs, ALL_rads, size):
    new_times = []
    new_vecs = []
    new_rads = []
    crops = []
    for i in range(len(ALL_times)):
        newtime, newvec, newrad, crop = value_cropper(ALL_times[i], ALL_vecs[i], ALL_rads[i], size)
...
def radius_calculation(ALL_rads):
    ...
    return avgoflist(final)

# after using guassian approx, gives user opportunity to remove unecessary TICs
# bad looking ones that aren't represented with the approx
def should_we_keep(time, vec, short_time, crop_vec):
    fig, ax1 = plt.subplots()
    title = "Cropping Intensity Time Curves (TICs) - Case 1 CABG Project"
    ax1.set_title(title)
    ax1.set_xlabel('time of perfusion acquisition [s]')
    ax1.plot(time, vec, label="GAUSS", color="purple")
...
def opportunity_to_remove(ALL_times, ALL_vecs, SHORT_times, CROP_vecs, names):
    new_times = []
    new_vecs = []
    new_names = []
    for i in range(len(ALL_times)):
        answer = should_we_keep(ALL_times[i], ALL_vecs[i], SHORT_times[i], CROP_vecs[i])
        if answer == "yes":
            ...
            pass
    return new_times, new_vecs, new_names
...
def running_velocity_extraction_NEW(TEST, size, parent_dir, typee, save=False, showw=True):
    ...
    plot_ALL_tics2(colours, typee, ALL_times, ALL_vecs, names, here, "RAW_TICS", save, showw)

    # first chance to remove bad TICS overall
    print("\n\n***\n*** READY TO REMOVE FIRST ROUND OF BAD TICS. ***\n***")
    SMOOTH_vecs = smooth_all(ALL_vecs, 5)
    ALL_times, ALL_vecs, names = opportunity_to_remove(ALL_times, ALL_vecs, ALL_times, SMOOTH_vecs, names)
    ...
    SHORT_times, CROP_vecs, CROP_rads, crops = timecrop_bad(ALL_times, ALL_vecs, ALL_rads, size)
    print("\n\n***\n*** DONE CROPPING. CROPPED DATA PRESENTED. ***\n***")
    plot_ALL_tics2(colours, typee, SHORT_times, CROP_vecs, names, here, "CROPPED_TICS", save, showw)
    
    # guassian the values
    print("\n\n***\n*** READY TO REMOVE BAD GAUSSIANS FROM CROPPED DATA. ***\n***")
    ALL_vecs = gauss_it(ALL_times, SHORT_times, CROP_vecs)
    ALL_times, ALL_vecs, names = opportunity_to_remove(ALL_times, ALL_vecs, SHORT_times, CROP_vecs, names)
    print("\n\n***\n*** DONE REMOVING. GOOD GAUSSIAN APPROXES PRESENTED. ***\n***")
    plot_ALL_tics2(colours, typee, ALL_times, ALL_vecs, names, here, "GAUSS_TICS", save, showw)
    ...
    # now calculating total velocity
    v = [v1, v2, v3, v4]
    v = avgoflist_pos(v)
    ...
    print("\n***\n'average vessel radius:", rad, "[pixels]\n***")

    # and now we'll do flowrate!
    print("\n\n***\n*** NOW CALCULATING FLOWRATE. ***\n***\n")
    flow1, flow2 = flowrate(v, rad, parent_dir)
...
# a_plane_over_time_LOTS(parent_dir, timebody_folder, TEST, spacing, all_time, size, here, speed, typee)
...
size, spacing = size_type_chooser(typee) 
running_velocity_extraction_NEW(TEST, size, parent_dir, typee, True, True)
