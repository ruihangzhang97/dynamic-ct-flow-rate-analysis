...
# main for this step
def run_tics_for_all_gifs(case, parent_dir, speed, frame_size, size, typee):
    gif_folder_path = parent_dir + "/gif/"
    reds_folder_path = parent_dir + "/reds/"
    path_folder_path = parent_dir + '/paths/'
    file_path = parent_dir + '/centerline_distance/distance.txt'
    distance_values = []
    counter = 0

    names = os.listdir(gif_folder_path)
    for j in range(1000):
        directory = parent_dir + '/gif/' + typee + str(j) + 'slice.gif'
    ...
    # def detect_intensity_changes(pixel_grid, boundary_pixels, vessel_intensity):
        # intensity_changes = []
        # true_indices = []
        # new_contour_coords = []
        # mean_neighbor_intensity = 0
        # boundary_intensity = 0
        # for x, y in boundary_pixels:
            # neighbors = get_neighbors_wide(pixel_grid, x, y)
            # neighbor_intensities = [(pixel_grid[ny, nx], nx, ny) for nx, ny in neighbors]

            # if any(mean_neighbor_intensity > boundary_intensity):
                # intensity_changes.append(True)
                # true_indices.append(len(intensity_changes) - 1)  # Store the index of the True value

                # Find the pixels in neighbors that have the same intensity as the vessel region
                # contour_pixels = [neighbor for neighbor, neighbor_intensity in zip(neighbors, neighbor_intensities) if neighbor_intensity == vessel_intensity]

                # Append the contour_pixels to the boundary_pixels list for the next frame
                # boundary_pixels.extend(contour_pixels)

            # else:
                # intensity_changes.append(False)

        # if not any(intensity_changes):
            # return np.zeros(len(pixel_grid), dtype=bool)

        # Interpolate the True indices to match the length of pixel_grid
        # interpolated_changes = np.interp(
            # np.arange(len(pixel_grid)),
            # true_indices,
            # np.ones(len(true_indices), dtype=float)
        # ).astype(bool)

        # return interpolated_changes

    def get_neighbors(pixel_grid, x, y):
        ...
        # Ensure theres 9 neighbors
        while len(neighbors) < 9:
            neighbors.append((x, y))

        return neighbors

    def calculate_vessel_intensity(pixel_grid, seed_x, seed_y):
        # Extract the pixel values in the vessel region using flood-fill
        height, width = pixel_grid.shape[:2]
        ...
        def flood_fill(x, y):
            ...
            if np.all(pixel_grid[y, x] == pixel_grid[seed_y, seed_x]):
                vessel_pixels.append(pixel_grid[y, x])
                flood_fill(x + 1, y)
                flood_fill(x - 1, y)
                flood_fill(x, y + 1)
                flood_fill(x, y - 1)

        flood_fill(seed_x, seed_y)

        # Calculate the intensity threshold as the mean intensity of the vessel region
        vessl_intensity = np.mean(vessel_pixels)
        return vessl_intensity

    def region_growing(pixel_grid, seed_x, seed_y):
        height, width = pixel_grid.shape[:2]
        # print("Pixel grid shape:", height, width)
        # print("Seed point coordinates:", seed_x, seed_y)
        visited = np.zeros((height, width), dtype=bool)
        boundary_pixels = []

        # Calculate the intensity threshold using the calculate_intensity_threshold function
        intensity_threshold = calculate_vessel_intensity(pixel_grid, seed_x, seed_y)
        print("Intensity Threshold:", intensity_threshold)

        seed_intensity = np.atleast_1d(pixel_grid[seed_y, seed_x])
        print("Seed Intensity:", seed_intensity)


        def region_grow2(x, y):
            if x < 0 or y < 0 or x >= width or y >= height:
                return
            if visited[y, x]:
                return
            ...
            if np.all(np.abs(pixel_grid[y, x] - pixel_grid[seed_y, seed_x]) <= intensity_threshold):
        ...
        region_grow2(seed_x, seed_y)
        # boundary_pixels = region_grow(seed_x, seed_y)
        # Remove the seed point from the boundary_pixels list
        boundary_pixels = [pixel for pixel in boundary_pixels if pixel != (seed_x, seed_y)]

        print("Boundary pixels:", boundary_pixels)  # Print the boundary pixels for debugging

        return boundary_pixels
    
    def find_outer_boundary(points):
        points = np.array(points)
        unique_points = np.unique(points, axis=0)
        if len(unique_points) < 3:
            print("Not enough unique points to construct the initial simplex.")
            return []

        try:
            hull = ConvexHull(unique_points, qhull_options='QJ QR0')
        except Exception as e:
            print("ConvexHull Error:", e)
            return []

        outer_boundary_indices = hull.vertices

        outer_boundary_points = unique_points[outer_boundary_indices]

        smooth_contour_points = []

        for i in range(len(outer_boundary_points)):
            p1 = outer_boundary_points[i]
            p2 = outer_boundary_points[(i + 1) % len(outer_boundary_points)]
            for t in range(101):  
                x = p1[0] + (p2[0] - p1[0]) * t / 100
                y = p1[1] + (p2[1] - p1[1]) * t / 100
                smooth_contour_points.append((int(x), int(y)))

        return smooth_contour_points

    def backward_update(pixelgrid4D, middle_frame_index, intensity_threshold, contours):
        # Prop the contour in the backward direction
        for i in range(middle_frame_index - 1, -1, -1):
            pixel_grid = pixelgrid4D[i]
            prev_boundary_pixels = contours[i + 1]
            neighboring_pixels = []

            for x, y in prev_boundary_pixels:
                neighbors = get_neighbors(pixel_grid, x, y)
                neighboring_pixels.extend(neighbors)
   
            new_boundary_pixels = []
            for nx, ny in neighboring_pixels:
                intensity = pixel_grid[ny, nx]
                if any(np.abs(intensity - intensity_threshold)) < 1:
                    new_boundary_pixels.append((nx, ny))

            contours[i] = new_boundary_pixels

            # Remove pixels from the current contour that do not satisfy the intensity condition anymore
            current_boundary_pixels = [j for i in [new_boundary_pixels, contours[i + 1]] for j in i] 

            for pixel in current_boundary_pixels:
                intensity = pixel_grid[pixel[1], pixel[0]]
                if any(np.abs(intensity - intensity_threshold)) > 1 and pixel in contours[i + 1]:
                    current_boundary_pixels.remove(pixel)
                    
            contours[i] = listremovedoubles(current_boundary_pixels)
            contours[i] = find_outer_boundary(contours[i])
            contours[i] = listremovedoubles(contours[i])

        return contours
    
    def forward_update_old(empirical_radius):
        updated_threshold_value = 0.2
        good_contour = False
        area = False
        empirical_area = empirical_radius ** 2 * math.pi
        if empirical_area  >= (1 + updated_threshold_value) * empirical_area and empirical_area  <= (1 - updated_threshold_value) * empirical_area:
            area = False
        else:
            area = True
        if empirical_radius >= (1 + updated_threshold_value) * empirical_radius and empirical_radius <= (1 - updated_threshold_value) * empirical_radius:
            good_contour = False
        else:
            good_contour = True
        return area and good_contour
  
    def forward_update(pixelgrid4D, middle_frame_index, intensity_threshold, contours):
        # Prop the contour in the forward direction
        for i in range(middle_frame_index + 1, len(pixelgrid4D)):
            pixel_grid = pixelgrid4D[i]
            prev_boundary_pixels = contours[i - 1]
            neighboring_pixels = []
    ...
    def distance_in_2d(point1, point2):
        x1, y1 = point1
        x2, y2 = point2
        return math.sqrt((x2 - x1)**2 + (y2 - y1)**2)

    def average_radius(contour_path):
        total_x = 0
        total_y = 0
        num_points = len(contour_path)
        ...
        sum_distances = 0
        for x, y in contour_path:
            sum_distances += distance_in_2d((x, y), (centroid_x, centroid_y))

        average_radius = sum_distances / num_points
        return average_radius
    
    def area(contour_path):
        # Gauss's area for polygons
        ...
        for i in range(num_points):
            x1, y1 = contour_path[i]
            x2, y2 = contour_path[(i + 1) % num_points]  
            total_area += (x1 * y2 - x2 * y1)

        area = abs(total_area) / 2
        return area

    def is_valid_contour(contour, empirical_radius):
        # set the error to be 15 percent for now: hardcoded value, may be different for different grafts
        error_allowed = 15
        
        contour_radius = average_radius(contour)
        max_allowed_radius = empirical_radius * (1 + error_allowed / 100)
        min_allowed_radius = empirical_radius * (1 - error_allowed / 100)
        is_allowed_radius = contour_radius >= min_allowed_radius and contour_radius <= max_allowed_radius

        contour_area = area(contour)
        max_allowed_area = ((empirical_radius ** 2) * math.pi) * (1 + error_allowed / 100)
        min_allowed_area = ((empirical_radius ** 2) * math.pi) * (1 - error_allowed / 100)
        is_allowed_area = contour_area >= min_allowed_area and contour_area <= max_allowed_area

        return is_allowed_radius and is_allowed_area

    def get_vessel_contour(pixelgrid4D):

        middle_frame_index = len(pixelgrid4D) // 2
        pixel_grid = pixelgrid4D[middle_frame_index]
        pixel_grid = pixel_grid.astype(np.uint8)
        center = clicker_man(pixel_grid)
        center_x = center[0]
        center_y = center[1]
        empirical_radius = 0 # change for different grafts

        boundary_pixels = region_growing(pixelgrid4D[middle_frame_index], center_x, center_y)
        print("Boundary pixels for middle frame:", boundary_pixels)

        contours = []
        for i in range(len(pixelgrid4D)):
            if i == middle_frame_index:
                contours.append(boundary_pixels)
            else:
                contours.append([])

        intensity_threshold = calculate_vessel_intensity(pixelgrid4D[middle_frame_index], center_x, center_y)

        # Track the last three contours, hard coded value
        prev_contours = [contours[middle_frame_index - 2], contours[middle_frame_index - 1], contours[middle_frame_index]]
        consecutive_same_contours = 0

        while consecutive_same_contours < 3:
            forward_update(pixelgrid4D, middle_frame_index, intensity_threshold, contours)
            
            # Check if the current contour is valid
            if not is_valid_contour(contours[middle_frame_index], empirical_radius): 
                consecutive_same_contours = 0  
                backward_update(pixelgrid4D, middle_frame_index, intensity_threshold, contours)  
                if contours[middle_frame_index] == prev_contours[2]:
                    consecutive_same_contours += 1
                else:
                    consecutive_same_contours = 0
                prev_contours.pop(0)
                prev_contours.append(contours[middle_frame_index])

        return contours
    
    def plot_my_contour(pixelgrid4D, center):
        locations = get_vessel_contour(pixelgrid4D)
        print(pixelgrid4D)
        ready = []
        for i in range(len(pixelgrid4D)):
            pixels = pixelgrid4D[i]
            pixels = pixels.astype(np.uint8)
            ready_locs = selected_shower(pixels, locations[i], center, typee = "single", showw = False)
            # print(locations[i])
            ready.append(ready_locs)
        return ready
    ...
    def the_whole_polarthing_mhmm(case, gif_path, parent_dir, speed, frame_size, size):
        ...
        cont = stitch_together(cont1, cont2)
        # cont = plot_my_contour(pixel_grid, centre)
        # print(cont)
        # print("**************************")
        ...
        reds = np.array(reds)
        print("Length of reds:", len(reds))  # Add this print statement
    ...
    for name in names:
        ...
        print("*******\nWORKING ON", name)
        pixelgrid4D = extract_gif(gif_path)
        contours = get_vessel_contour(pixelgrid4D)
    ...
    with open(file_path, "w") as textfile:
        textfile.write("Distance between Contour and Centerpath [pixels]\n")
...
def running_velocity_extraction_NEW(case, TEST, size, parent_dir, typee, save=False, showw=True):
    ...
    def separator(slices):
        for i in range(len(slices)):
            length = len(slices)
        first_half = slices[:length/2]
        second_half = slices[length/2:]
        first_half = first_half.reverse()

        # this function dominates the order in which the slices are generated and the starting point which is supposed
        # to be as accurate as possible as the center gets updated based on the first slice
        return first_half, second_half

    def visualization_intersection(intersection_point, slices, centerpath, typee):
        # typee should be a string, the name of the vessel
        parent_dir = '/Users/ruihangzhang/Desktop/TESTER/CASE1' + typee + '.pth'
    
    def scalar_projection(plane, centerline):
        centerline = centerline[:2]
        plane = plane[:1]
        return centerline, plane
    
    def normal_vector(slices, centerpath, typee, distance):
        predict = 0
        slice_length = slices[2]/2
        lower_bound = distance
        slices_x_y = distance + slices[2] / 2

        result = running_velocity_extraction_NEW(visualization_intersection(0.5, slices, centerpath, typee))

        for slice in slices:
            lower_bound = slice/2
            slices_x_y = lower_bound + slices_x_y + slice_length

        # visualization_intersection(result, predict)
...
def create_slice_gifs_along_MANY_vessels(case, list_typees, pparent_dir, timebody_folder, centrelines):
    ...
    for typee in list_typees:
        ...
        for i in choices:
            ...
            # contours, success = get_vessel_contour(pixelgrid4D, choose, here)
            # if success:
               #  print(contours)
               #  print("Sucess!!")
            # else:
...
def segment_all_slices_for_TICS(case, typee, pparent_dir, centrelines):
    ...
    size, spacing = size_type_chooser(typee) 
    avgrad = run_tics_for_all_gifs(case, parent_dir, speed, frame_size, size, typee)
...
def run_TOF_for_results(case, typee, pparent_dir, centrelines):
    ...
    def all_rad_on_one(parent_dir):
        frame_size = frame_size_chooser(typee)
        counter = 0
        time_values = []
        rad_values = []
        saved_dir = parent_dir + '/TICPLOT_ANALYSIS/'
        for j in range(1000):
            directory = parent_dir + '/' + typee + str(j) + 'slice/radresults.txt'
            if os.path.exists(directory):
                counter += 1
                with open(directory, 'r') as f:
                    lines = f.read().split('\n')
                    r_index = lines.index('average radius of the vessel lumen [pixels]')
                    for i in range(1, len(lines)):
                        if i < r_index - 1:
                            time_values.append(lines[i])
                        if i > r_index and i < (len(lines) - 1):
                            rad_values.append(lines[i])
                time_values = [float(value) for value in time_values]
                rad_values = [float(value) for value in rad_values]
                plt.plot(time_values, rad_values, label = "slice " + str(j))
                time_values = []
                rad_values = []
        title = "Time Radius Curve (TIC) of All Slices"
        plt.xlabel("time of perfusion acquisition [s]")
        plt.ylabel("average radius of the vessel lumen [pixels]")
        plt.title(title)
        plt.legend()
